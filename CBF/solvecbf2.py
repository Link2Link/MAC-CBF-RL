...
def solvecbf(speed_control, angle_control, cbf_obs:CBFObservation):
    ...
    constraints = [ddh_r + alpha_1*dh_r + alpha_0*(h_r - 0.5*(r1**2)) >=0,
                   -ddh_r - alpha_1*dh_r - alpha_0*(h_r - 0.5*(r2**2)) >=0,
                   ]
    cars_constraints = []
    hc = []
    dhc = []
    ...
    for i in range(num):
        constraints.append(ddhc[i] + alpha_1*dhc[i] + alpha_0*hc[i] >=0)

    obj = Minimize(square(u_a - speed_control) + 10 * square(u_w - angle_control))
